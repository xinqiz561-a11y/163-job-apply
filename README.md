...
发送 → 私有 JSONL 结果记录”封装成可复用流程。项目默认支持 163 邮箱，暂不
包含招聘网站自动填表；如需使用 Gmail、Outlook 或其他邮箱，可让 GPT 根据
对应邮箱服务商的认证与发送规则自行调整适配。

## 核心用途

在投递邮箱已经核验、用户已在本机安全配置 163 客户端授权码的前提下，
GPT 可在后台生成并检查求职邮件，完成附件校验和重复投递检查，并在用户
一键确认后通过 163 SMTP 代发简历。该项目为 GPT 提供了可复用的 163 邮箱
发送适配层；结合 GPT 自行检索匹配岗位和核验官方招聘邮箱的能力，可形成
“岗位检索 → 匹配评估 → 邮件预览 → 一键确认 → 发送 → 去重留档”的完整
投递闭环。岗位检索和发送准备可以自动完成，真实发送始终保留用户确认环节。
